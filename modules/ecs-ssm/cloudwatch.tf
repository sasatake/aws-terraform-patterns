...
resource "aws_cloudwatch_event_target" "ecssrv_scale_in" {
  ...
  arn       = "${replace(aws_ssm_document.ecssrv_scale_in.arn, "document/", "automation-definition/")}:$DEFAULT"
...
resource "aws_cloudwatch_event_target" "ecssrv_scale_out" {
  ...
  arn       = "${replace(aws_ssm_document.ecssrv_scale_out.arn, "document/", "automation-definition/")}:$DEFAULT"
